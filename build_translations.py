...
BACKUP_PATH = "config/hivekeeper_entities_backup.yaml"
    ...
    for entity in config.get("entities", []):
        for field in entity.get("fields", []):
            ...
            if options and isinstance(options, list):
                new_options = []
                for opt in options:
                    if isinstance(opt, (int, float, bool)):
                        new_options.append(str(opt))
                        changed += 1
                    else:
...
def main():
    ...
    with open(BACKUP_PATH, "w", encoding="utf-8") as backup:
        yaml.dump(config, backup, allow_unicode=True, sort_keys=False)
        print(f" Backup geschrieben → {BACKUP_PATH}")

    changed = stringify_options(config)

    if changed > 0:
        with open(CONFIG_PATH, "w", encoding="utf-8") as out:
            yaml.dump(config, out, allow_unicode=True, sort_keys=False)
        print(f" {changed} Optionen als Strings gespeichert → {CONFIG_PATH}")
    else:
        print(" Keine Optionen mussten umgewandelt werden.")

    generate_translations(config)
